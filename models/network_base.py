...
class Network(object):
    ...
    def split_conv2d(self,inputs,kernel_size,rate,out_channels,name=None,padding='SAME',both_atrous=False):
        ...
        if both_atrous:
           outA=tf.nn.atrous_conv2d(inputs,kernelA,rate[0],padding=padding)
           outB=tf.nn.atrous_conv2d(inputs,kernelB,rate[1],padding=padding)
        else:
    ...
    def unit_0(self,x,out_channels,block,unit):
        ...
        return b_u_out    
    def unit_v1a(self,x,out_channels,stride,block,unit,shortcut=False):
        
        with tf.variable_scope('block%d/unit_%d/bottleneck_v1'%(block,unit)):
             b_u_1r_cout=self.conv_batchN_relu(x,1,stride,out_channels/4,name='conv1')
             b_u_3_cout=self.aconv_batchN_relu(b_u_1r_cout,3,2,out_channels/4,name='conv2')
    ...
    def unit_v4(self,x,out_channels,block,unit,shortcut=False,dropout=False,rate=[2,2]):
        with tf.variable_scope('block%d/unit_%d/bottleneck_v1'%(block,unit)):
             b_u_1r_cout=self.conv_batchN_relu(x,1,1,out_channels/4,name='conv1')

          
             with tf.variable_scope('conv2'):
                  b3_u3_3_cout=self.split_conv2d(b_u_1r_cout,3,rate,out_channels/4,both_atrous=True)
                  b3_u3_3_bnout=self.batch_norm(b3_u3_3_cout)
                  b3_u3_3_ract=tf.nn.relu(b3_u3_3_bnout)
